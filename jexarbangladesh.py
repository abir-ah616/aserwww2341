    ...
    def sockf1(self, tok, online_ip, online_port, packet, key, iv):
        ...
        while True:
            ...
            if "0f00" in data2.hex()[0:4]:
                ...
                if asdj == 15:
                    if tempdata == "OFFLINE":
                        tempdata = f"The Account is {tempdata}"
                    else:
                        ...
                        if "INSQUAD" in tempdata:
                            ...
                            tempdata = f"UID : {idplayer1}\nStatus : {tempdata}\nLeader UID : {idleader1}"
                        else:
                            tempdata = f"UID : {idplayer1}\nStatus : {tempdata}"
                    statusinfo = True 
            ...
            if "1200" in data.hex()[0:4]:
                ...
                if "8" in parsed_data["5"]["data"] and "data" in parsed_data["5"]["data"]["8"]:
                    ...
                    if uexmojiii == "DefaultMessageWithKey":
                        ...
                        clients.send(
                            self.GenResponsMsg(
                            f"""[FFFFFF][b][c]✨Welcome to our Bot✨[/b]

[FFFFFF][c]To Get full list of working commands, [c]send this:  

[00FF00][b][c]/🤔help[/b]

[FFD700][c]For any kind of information:  

[b][c][40E0D0]Facebook: Hasanuzzaman Oppy[/b]

[b][c][1E90FF] Insta: @mroppy21

[b][c][40E0D0]Discord: @x_forsaken[/b]""",uid
                            )
                        )
                else:
                    pass  
...
[b][i][00FF00]Facebook: Hasanuzzaman Oppy[/b]

[b][c][1E90FF]Instagram : @mroppy21[FFFFFF]
 
[b][c][FF6347]Discord : @x_forsaken[/b]""", uid
                    )
                )
            

            if False and "1200" in data.hex()[0:4] and b"/x" in data:
                try:
            ...
            if "1200" in data.hex()[0:4] and b"/3" in data:
                ...
                if uid:
                    clients.send(
                        self.GenResponsMsg(
                            f"[C][B][1E90FF]-----------------------------\n\n\n\n3 Player Team Has been created, accept the invite asap!\n\n\n\n-----------------------------",
                            uid
                        )
                    )
            ...
            if "1200" in data.hex()[0:4] and b"/5" in data:
                ...
                if uid:
                    clients.send(
                        self.GenResponsMsg(
                            f"[C][B][1E90FF]-----------------------------\n\n\n\n5 Player Group has been created, accept the invite asap!\n\n\n\n-----------------------------",
                            uid))
            ...
            if "1200" in data.hex()[0:4] and b"/6" in data:
                ...
                if uid:
                    clients.send(
                        self.GenResponsMsg(
                  f"[C][B][1E90FF]-----------------------------\n\n\n\n6 Player Group has been created, accept the invite asap!\n\n\n\n-----------------------------",
                            uid))
            ...
            if "1200" in data.hex()[0:4] and b"/status" in data:
                try:
                    ...
                    if room_data:
                            ...
                            if statusinfo == True:
                                if "IN ROOM" in tempdata:
                                    try:
                                        ...
                                        sleep(0.5)
                                        if tempdata1 is not None and tempdata1 != "None":
                        ...
                        clients.send(self.GenResponsMsg("[C][B][FF0000] Please enter UID!", uid))  
                except Exception as e:
            ...
            if "1200" in data.hex()[0:4] and b"/inv" in data:
                ...
                if room_data:
                    ...
                    if numsc1 is None:
                        ...
                        if int(numsc1) < 3 or int(numsc1) > 6:
                            ...
                            clients.send(
                        self.GenResponsMsg(
                            f"[C][B][00ff00]The player has been invited and also invited you to the group, accept asap! ", uid
                        )
                    )
                ...
                clients.send(
                     self.GenResponsMsg(
                         f"[C][B] [FF00FF]Bot is now solo", uid
                     )
                 )
                ...
                if room_data:
                    ...
                    if player_id.isdigit():
                        ...
                        if "IN ROOM" in tempdata:
                            ...
                            clients.send(
                                self.GenResponsMsg(
                                    f"[C][B][00ff00]Room join spam request is being send to UID {fix_num(player_id)} ! ", uid
                                )
                            )
                            ...
                            clients.send(
                                self.GenResponsMsg(
                                    f"[C][B] [00FF00]Request spam completed! ✅", uid
                                )
                            )
                        else:
                            clients.send(
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                f"[C][B] [FF00FF]Please enter the uid of the player not!", uid
                            )
                        )   
                    ...
                    clients.send(
                        self.GenResponsMsg(
                            f"[C][B] [FF00FF]Please send the uid of the player !", uid
                        )
                    )   
            ...
            if "1200" in data.hex()[0:4] and b"WELCOME TO [FFFFF00] MY BOT" in data:
                pass
            else:
             
                    if False and "1200" in data.hex()[0:4] and b"/spam" in data:
                    ...
                    if False and "1200" in data.hex()[0:4] and b"/visit" in data:
            ...
            self.GenResponsMsg(
                f"{generate_random_color()}جارِ إرسال 1000 زيارة إلى {fix_num(player_id)}...", uid
                    ...
                    if False and "1200" in data.hex()[0:4] and b"/info" in data:
                        try:
    ...
    f"[B][FFA500]• Bio: [FFFFFF]{bio}\n"
                    ...
                    if False and "1200" in data.hex()[0:4] and b"/likes" in data:
                    ...
                    if "1200" in data.hex()[0:4] and b"/check" in data:
                           try:
                                ...
                                clients.send(
                                self.GenResponsMsg(
                            f"{generate_random_color()}Checking UID", uid
                        )
                    )
                                if len(command_split) > 1:
                            ...
                            f"{generate_random_color()}Player ID : {player_id}\n"
                            f"Status: {status}"
                        )
                    ...
                    if "1200" in data.hex()[0:4] and b"/help" in data:
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                        f"""[B][C][FFFF00] WELCOME TO [ffffff]To My FF BOT  !! 

                                ────────────
        
[40E0D0] /🤔status [uid] ->  [C][B][FFFFFF]Check Players In-Game Status
 
                ────────────

[00FF00] /🤔check [uid] -> [C][B][FFFFFF]Check Players Ban Status

────────────

[339966] /🤔admin -> [C][B][FFFFFF]About Admin
""", uid
                            )
                        )
                        time.sleep(0.5)
...
[FF0066] /🤔inv [uid] -> [C][B][FFFFFF] Invite Any Player

────────────

[00FF00] /🤔3 -> [C][B][FFFFFF] Create 3 Player Squad

────────────

[00B300] /🤔5 -> [C][B][FFFFFF] Create 5 Player Squad

────────────

[006600] /🤔6 ->[C][B][FFFFFF] Create 6 Player Squad
""", uid
                            )
                        )
                        time.sleep(0.5)
                        clients.send(
                                    self.GenResponsMsg(
                                        f"""            ────────────
[40E0D0] /🤔room [uid] -> [C][B][FFFFFF]Send Room Spam Request

────────────

[00FF00] /🤔attack [tc]-> [C][B][FFFFFF] Send Team Join Spam
...
""", uid
                    ...
                    if False and "1200" in data.hex()[0:4] and b"/ai" in data:
            ...
            if False and '1200' in data.hex()[0:4] and b'/lag' in data:
                try:
            ...
            if '1200' in data.hex()[0:4] and b'/attack' in data:
                try:
                    ...
                    if not command_parts:
                        clients.send(self.GenResponsMsg("[C][B][FF0000]Please Enter UID to send Team Join Spam like this \n/attack [TeamCode]", uid))
                        continue
                    ...
                    clients.send(
                        self.GenResponsMsg(f"[C][B][FFA500]Team Join Spam is started in this team code {team_code} ...", uid)
                    )
                    ...
                    clients.send(
                        self.GenResponsMsg(f"[C][B][00FF00]Team Join Spam has been completed in team code {team_code}!", uid)
                    )
            ...
            if False and "1200" in data.hex()[0:4] and b"/start" in data:
                try:
                    # تقسيم البيانات القادمة بعد الأمر
